...
M1 implements deterministic configuration, Node network denial, offline Chromium with audited interception, ignored artifacts, disabled install scripts, and least-privilege deterministic CI. No Automation Exercise or other live target contact is authorized. Live-target controls below are future requirements, not implemented integrations. [AGENTS.md](../AGENTS.md) governs authorization; [roadmap](roadmap.md) controls progression.
...
Required deterministic checks deny outbound network access, including setup and teardown; browser interception must cover the initial document and resources, with service workers blocked. Live mode requires explicit activation and validated HTTPS origins/endpoints. Reject unapproved redirects and third-party requests. Exact allowed hosts/paths must be approved before the first live run; M1 does not inspect the site to discover them.

Before the first authorized live run, re-check Automation Exercise Terms of Service and `robots.txt`, record the check date and relevant restrictions, and confirm the approved scenario and allowlist remain permitted. Neither check grants authorization by itself. Resolve uncertain or conflicting restrictions before live traffic. This check is deferred; M1 must not contact the target.
...
Retention policy is seven days maximum for local raw diagnostics and future CI failure artifacts; delete earlier when no longer needed. CI uploads are disabled. Local reports are overwritten by subsequent runs; users must delete remaining diagnostics within seven days. No automatic local retention scheduler is implemented. CI upload remains off until redaction and publication risks are reviewed. Upload only explicit run-owned sanitized paths, never the workspace or home directory. Public-repository artifact access must not be treated as confidential. Any retained portfolio example requires separate review and publication authorization; never use raw traces as public sample evidence by default.
